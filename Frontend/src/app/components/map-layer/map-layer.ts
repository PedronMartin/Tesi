import { Component, AfterViewInit, PLATFORM_ID, Inject, Input, NgModule } from '@angular/core';
    ...
    if (isPlatformBrowser(this.platformId)) {
      import('leaflet-draw').then(() => {
        ...
        const bounds = MapLayerComponent.captureData[0];
            if (bounds) {
              //ottengo i quattro angoli dai bounds
    ...
    if (isPlatformBrowser(this.platformId) && this.map && this.featureGroup) {
      this.featureGroup.eachLayer((layer: any) => {
        if (layer instanceof this.L.Polygon) {
          const latLngs = layer.getLatLngs()[0]; //ottiengo i vertici del poligono
